...
for spec in permutations(range(N, 2*N)):
    queues = [[x] for x in spec]
    queues.append(queues[0])
    queues[0].append(0)

    workers = [run(init_code[:], inq=queues[i], outq=queues[i+1]) for i in range(N)]
    for _ in zip_longest(*workers):
        pass
